 ...
 * proxy.js - 代理连接模块
 * 
 * 包含 SOCKS5、HTTP CONNECT、HTTPS CONNECT (TLS)、TURN (STUN/TURN) 和 SSTP 代理连接实现。
 * 原始代码来自 _worker.js:
 *   - 第 2582-2783 行: SOCKS5/HTTP/HTTPS CONNECT 代理
 *   - 第 3477-3747 行: TURN 代理 (STUN/TURN Allocate/Connect/Bind)
 *   - 第 3748-4181 行: SSTP 代理 (SSTP over PPP over TCP)
 * 
 * @module proxy
 */

import { log, state } from './state.js';
import { 数据转Uint8Array, 拼接字节数据, 有效数据长度, isIPHostname, stripIPv6Brackets } from './utils.js';
import { TlsClient, textEncoder, textDecoder } from './tls.js';

// DoH查询 和 拼接字节数据 需要由上层模块提供或后续模块导出
// 临时声明: DoH查询 将在 dns.js 模块中导出
// import { DoH查询 } from './dns.js';

///////////////////////////////////////////////////////SOCKS5/HTTP函数///////////////////////////////////////////////
export async function socks5Connect(targetHost, targetPort, initialData, TCP连接) {
...
export async function httpConnect(targetHost, targetPort, initialData, HTTPS代理 = false, TCP连接) {
	const { username, password, hostname, port } = state.parsedSocks5Address;
...
export async function httpsConnect(targetHost, targetPort, initialData, TCP连接) {
	const { username, password, hostname, port } = state.parsedSocks5Address;
	const encoder = new TextEncoder();
	const decoder = new TextDecoder();
	let tlsSocket = null;
